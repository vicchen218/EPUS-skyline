...
if __name__ == "__main__":
    ...
    inslist = (3,4,5,6,7,8,9,10)
    rowname = ('Instance','Node Number', 'Mean Edge Processing Time', 'Max Edge Processing Time', 'Server Processing Time',
    ...
    for runtime in range(1):
        for ins in inslist:#for ins test
            for num in rank:
                ### localedge
                edgenum = num
                etmax = []
                ...
                df.loc[row_index, 'Instance'] = ins
                df.loc[row_index, 'Node Number'] = num
                print("ins is",ins)
                for k in range(edgenum):
                    eid = str(k)
                    usky = slideUPSky(2, ins, 5, [0,1000], wsize=300)
                ...
                sdatalist =[]
                #r.write('-- Transmission cost with ins {a}--\n'.format(a=ins))
                for k in range(edgenum):
                    # print("\n\n")
                    sdata =0
                ...
                sw = 300*edgenum
                skyServer = servePSky(2, ins, 5, [0,1000], wsize=sw)
                ...
                filename=f"new_EPUS-Distributed_Instance_test_{runtime}.csv"      
